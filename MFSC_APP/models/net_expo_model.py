...
class Net_Expo_Model:
    def  __init__(self):
        self.filepath = r"D:\MFSC_PROGRAM\UAT\MFSC_APP\asset\net_expo_template.xlsx"
        self.data = None
    ...
    def load_data(self):
        try:
            ...
            self.data = pd.DataFrame(columns=['REF_NO','EFFECTIVE_DATE','COMP_CODE','BEN_DESC','NET_LIAB','USER_UPLOAD','UPLOAD_DATE','DELETE_FLAG',"ID"])
    ...
    def export_data(self):
        ...
        export_columns = ['REF_NO','EFFECTIVE_DATE','EXP_CODE','CLIENT_CODE','NET_EXPOSURE',"VAR",'USER_UPLOAD','UPLOAD_DATE','DELETE_FLAG']
        try:
    ...
    def import_csv(self):
        """Allow the user to import data from a CSV file."""
        root = Tk()
        root.withdraw()  # Hide the root window
        ...
        try:
            imported_data = pd.read_csv(file_path, sep='|', encoding='utf-8')
            # Ensure imported data matches the template structure
            required_columns = ['REF_NO', 'EFFECTIVE_DATE', 'COMP_CODE', 'NET_LIAB', 'USER_UPLOAD', 'UPLOAD_DATE', 'DELETE_FLAG']
            if list(imported_data.columns) != required_columns:
                print("The imported file does not match the required template structure.")
                return

            # Append imported data to the existing dataset
            imported_data["ID"] = [str(uuid.uuid4()) for _ in range(len(imported_data))]
