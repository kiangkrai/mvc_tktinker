...
class Financial_liabl(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.input_section = ttk.LabelFrame(self,text='Financial Liability Page')
        ...
        self.COMP_CODE_lb = ttk.Label(self.input_section,text="COMP_CODE")
        self.COMP_CODE_lb.grid(row=3, column=0, padx=5, pady=10)
        self.COMP_CODE_input = ttk.Entry(self.input_section)
        self.COMP_CODE_input.grid(row=3, column=1, padx=5, pady=10)
        self.COMP_CODE_input.bind("<KeyRelease>", self.validate_COMP_CODE)

        self.NET_LIAB_lb = ttk.Label(self.input_section,text="NET_LIAB")
        self.NET_LIAB_lb.grid(row=4, column=0, padx=5, pady=10)
        self.NET_LIAB_input = ttk.Entry(self.input_section)
        self.NET_LIAB_input.grid(row=4, column=1, padx=5, pady=10)
        self.NET_LIAB_input.bind("<FocusOut>", self.validate_NET_LIAB)

      
        self.DELETE_FLAG_lb = ttk.Label(self.input_section,text="DELETE_FLAG")
        self.DELETE_FLAG_lb.grid(row=5, column=0, padx=5, pady=10)
        self.DELETE_FLAG_input = ttk.Combobox(self.input_section, values=["DELETE"],state="readonly")
        self.DELETE_FLAG_input.grid(row=5, column=1, padx=5, pady=10)
        ...
        self.import_btn.grid(row=5, column=6, padx=10, pady=5, sticky="w")
        ...
        cols = ('REF_NO','EFFECTIVE_DATE','COMP_CODE','NET_LIAB','USER_UPLOAD','UPLOAD_DATE','DELETE_FLAG',"ID")
...
#----------------------validate------------------
    ...
    def validate_COMP_CODE(self, event):
            """Validate COM_CODE input: maximum 10 characters, alphanumeric, and specific special characters."""
            input_data = self.COMP_CODE_input.get()
            allowed_pattern = r'^[a-zA-Z0-9 _]*$'
            if len(input_data) > 10:
                self.COMP_CODE_input.delete(10, tk.END)
                messagebox.showwarning("Validation Error", "COM_CODE cannot exceed 10 characters.")
            elif not re.match(allowed_pattern, input_data):
                self.COMP_CODE_input.delete(0, tk.END)
                messagebox.showwarning("Validation Error", "COM_CODE can only contain alphanumeric characters, '/', '_', and '|'.")

    def validate_NET_LIAB(self, event):
            """Validate PER_WEIGHT: must be numeric with a 5,2 format."""
            input_data = self.NET_LIAB_input.get()
            try:
                value = float(input_data)
                integer_part, decimal_part = input_data.split(".")
                if len(integer_part) > 20 or len(decimal_part) > 2 :
                    raise ValueError("NET_LIAB must follow the format 20,2 (e.g., 12345.67).")
            except ValueError:
                self.NET_LIAB_input.delete(0, tk.END)
                messagebox.showwarning("Validation Error", "NET_LIAB must be numeric and follow the format 20,2.")
...
if __name__ == "__main__":
    ...
    benchmark_view = Financial_liabl(root)
